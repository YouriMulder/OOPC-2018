...
public:
	...
	bool operator==(const rational & rhs) const {
		...
		return (lhs_tmp.counter == rhs_tmp.counter) &&
		       (lhs_tmp.denominator == rhs_tmp.denominator);
	}
	...
	friend std::ostream & operator<<(std::ostream & lhs, const rational & rhs) {
		return lhs
		       << '['
		       << std::hex<< std::internal << std::setfill('0') << std::showbase << std::setw(6) << rhs.counter
		       << ','
		       << std::hex<< std::internal << std::setfill('0') << std::showbase << std::setw(6) << rhs.denominator
		       << ']';
	}
	...
	/// multiply a rational by a rational
	/// \details
	/// This oprator*= multiplies a rational value by a rational value
	rational operator*=(const rational & rhs) {
